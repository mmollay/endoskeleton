...
SCRIPT;
     ...
     * Cuts from one HTML comment marker to the next, checks for visible text.
     */
    private function removeEmptySections(string $html): string
    {
        ...
        for ($c = count($comments) - 1; $c >= 0; $c--) {
            $commentText = $comments[$c][0];
            $commentStart = $comments[$c][1];

            // Check if this comment marks an optional section
            $isOptional = false;
            foreach ($optional as $label) {
                if (strpos($commentText, $label) !== false) {
                    $isOptional = true;
                    break;
                }
            }
            ...
            // Section runs from this comment to the next comment (or <!-- Footer -->)
            $sectionStart = $commentStart;
            if ($c + 1 < count($comments)) {
                $sectionEnd = $comments[$c + 1][1];
            } else {
                $sectionEnd = strlen($html);
            }

            $sectionHtml = substr($html, $sectionStart, $sectionEnd - $sectionStart);
            $text = trim(strip_tags($sectionHtml));

            if ($text === '') {
                // Remove the section including leading whitespace on its line
