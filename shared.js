/* ═══════════════════════════════════════════════════════════════════════════
   SSI WebGenerator Endoskeleton — shared.js v4.0.0
...
(function () {
  ...
  var defaultConfig = {
    ...
    cookieText:
      "Diese Website verwendet Cookies, um Ihnen ein optimales Erlebnis zu bieten.",
    footer: {
      tagline: "",
      credit: true,
    },
  ...
  function buildNavigation() {
        ...
        item.children.forEach(function (child) {
          subHtml +=
            ...
            (isActive(child.href)
              ...
              : "") +
            ">" +
            escapeHtml(child.text || child.label) +
            "</a></li>";
        });
        subHtml += "</ul>";
        linksHtml +=
    ...
    CONFIG.nav.forEach(function (item) {
      mobileHtml +=
        ...
        (item.cta
          ? ' class="nav-cta" style="text-align:center;margin-top:1rem;border-radius:var(--radius-sm);"'
          : "") +
        ">" +
        escapeHtml(item.text || item.label) +
        "</a>";
    ...
    el.innerHTML =
      ...
      '<div class="nav-inner">' +
      '<a href="index.html" class="nav-logo" aria-label="' +
      escapeHtml(CONFIG.name) +
      ' Startseite">' +
      logoHtml +
      "</a>" +
    ...
    if (burger && mobile) {
      burger.addEventListener("click", function () {
        ...
        burger.setAttribute(
          "aria-label",
          open ? "Navigation schliessen" : "Navigation öffnen",
        );
    ...
    if (nav) {
      ...
      function applyNavState(current) {
        ...
        var behavior =
          window.SITE_CONFIG && window.SITE_CONFIG.navBehavior != null
      ...
      window._applyNavState = function () {
        applyNavState(window.scrollY);
      };
    }
  }
  ...
  function buildFooter() {
    ...
    var logoHtml = CONFIG.logo
      ? '<div class="footer-logo"><img src="' +
        ...
        '" alt="' +
        escapeHtml(CONFIG.name) +
        ' Logo" width="140" height="36"></div>'
      : '<div class="footer-logo"><strong>' +
        escapeHtml(CONFIG.name) +
        "</strong></div>";
    ...
    el.innerHTML =
      ...
      '<a href="' +
      escapeHtml(CONFIG.legal.impressum) +
      '">Impressum</a>' +
      '<a href="' +
      escapeHtml(CONFIG.legal.datenschutz) +
      '">Datenschutz</a>' +
      "</div>" +
      "</div>" +
  ...
  function loadModules() {
    ...
    modules.forEach(function (mod) {
      ...
      script.onerror = function () {
        console.warn(
          '[Endoskeleton] Module "' + mod + '" konnte nicht geladen werden.',
        );
      };
      ...
      fetch(form.action || "contact.php", {
        ...
        .then(function (data) {
          ...
          if (data.ok) {
            ...
            msg.style.cssText =
              "color:var(--error);padding:var(--space-md);background:rgba(192,57,43,0.1);border-radius:var(--radius-sm);margin-top:var(--space-md);font-size:var(--fs-sm);";
            msg.textContent =
              data.error ||
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (footerStyleInit && footerStyleInit !== "dark") {
      ...
      if (footerEl) {
        if (footerStyleInit === "light")
          footerEl.classList.add("site-footer--light");
